  ...
  return new Promise((resolve, reject) => {
    ...
    devChild.on("exit", (code) => {
      if (spawned && code !== null && code !== 0) {
        reject(new Error(`next dev exited with code ${code}. ${stderr.slice(-500)}`.trim()));
      }
    });
...
export async function startE2eServer() {
  ...
  let baseUrl = process.env.E2E_BASE_URL?.replace(/\/$/, "") ?? null;
  let didSpawn = false;

  if (!baseUrl) {
    baseUrl = await findHealthyServer(PROBE_URLS);
  }
  ...
  if (!baseUrl) {
    ...
    try {
      ...
      const fallback = await findHealthyServer(["http://127.0.0.1:3000", spawnUrl]);
      if (fallback) {
        if (child) {
          child.kill("SIGTERM");
          child = null;
  ...
  await waitForLogin(baseUrl, adminEmail, password);
  ...
  console.log(`[e2e] Using ${baseUrl}${didSpawn ? " (spawned for tests)" : " (existing server)"}`);

  return { baseUrl, didSpawn };
}

export async function stopE2eServer(didSpawn) {
  removeE2eRuntimeEnv();
  if (!didSpawn || !child) return;
